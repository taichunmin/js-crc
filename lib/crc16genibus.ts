 ...
 */
const xorout = 0xFFFF
export default function crc16genibus (buf: Uint8Array = new Uint8Array(), prev: number = 0x0000): number {
  u16[0] = prev ^ xorout // revert xorout
  for (const b of buf) u16[0] = POLY_TABLE[(u16[0] >>> 8) ^ b] ^ (u16[0] << 8)
  return u16[0] ^ xorout // revert xorout
}

setObject(globalThis, ['taichunmin', 'crc', 'crc16genibus'], crc16genibus)
