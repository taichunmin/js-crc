import { setObject, u8 } from './common2'

const POLY_TABLE = new Uint8Array([
  ,
])

/**
 * - poly: 0x2F
 * - initial: 0x00
 * - xorout: 0x00
 * - refin: false
 * - refout: false
 */
export default function crc8opensafety (buf: Uint8Array = new Uint8Array(), prev: number = 0x00): number {
  u8[0] = prev
  for (const b of buf) u8[0] = POLY_TABLE[u8[0] ^ b]
  return u8[0]
}

setObject(globalThis, ['taichunmin', 'crc', 'crc8opensafety'], crc8opensafety)
